...
execute:
  cache: false # true refresh false
  warning: false
knitr:
  opts_chunk:
    out.width: "100%"
...
The data used for these analyses are stored in an `R Bioconductor MultiAssayExperiment` object which contains the QCed and pre-processed data. The code and description of how raw data have been processed and QCed is available on the [`VIBRANT-0-QC-and-preprocessing` GitHub repository](https://github.com/lasy/VIBRANT-0-QC-and-preprocessing). 
...
figure2b <- 
  ...
  plot_layout(
    ...
    BC
    ",
    ncol = 2, widths = c(2, 1.4), heights = c(2.65, 1)
    ) +
  plot_annotation(tag_level = "A") + 
  theme(
    plot.tag = element_text(
...
ggsave(
  figure2b, filename = "figures/Figure2b_colonization.pdf",
  width = 12, height = 12.5, dpi = 300, units = "in"
)
...
figure4 <- 
  ...
  g_colonized_among_exposed + guides(col = "none") +
  plot_layout(ncol = 1, nrow = 2, heights = c(2, 1.2)) +
  plot_annotation(tag_level = "A") + 
  theme(
    plot.tag = element_text(
      face = "bold",
...
# figure 4
ggsave(
  figure4, filename = "figures/Figure4_strains.pdf",
