...
# depuis la mise a jour de la table exposures (dfseq replace by visit_code --> ne fonctionne plus)
# il faut prendre la table exposures Rdata plutot que via le mae 
...
--> reprendre le colonise at -> filtrer ceux pour lesquels c'est TRUE entre visite 1300 et 1500 et puis regarder combien on une proportion >0.5
```{r}
...
## ✅ Nugent score : LBP colonization in participants with BV post-MTZ
...
Across all active arms combined, YY participants achieved detection of a single LBP strain by metagenomic sequencing at least once in the first 5 weeks. Approximately 40-60% of participants that had detected LBP strains at 5 weeks, still had detection at 12 weeks demonstrating prolonged colonization after a short initial treatment course. 

> "a single LBP" est strict (une seule souche détectée et pas deux") ou c'est une façon de dire au moins une souche ? 
