...
table1_data <- 
  ...
  dplyr::left_join(colData(mae) |> as_tibble() |> select(pid, site, ITT, mITT, PP) |> distinct(), by = "pid") |> 
  filter(mITT) |> 
  select(pid, site, age_at_consent, race, ethn, cut_size_meals, eat_less, hungry_did_not_eat, sexual_partners_lifetime, sexual_partners_past_month, contraception_method, group4) |> 
  mutate(
...
table1_data <- 
  colData(mae) |> as_tibble() |> select(pid, site, ITT, mITT, PP) |> distinct() |> 
  left_join(
    bind_rows(
    mae@metadata$crf_data_clean$crf1 |> select(pid, age, race, ethn) |> distinct(),
    mae@metadata$crf_data_clean$crf101 |> select(pid,  age, race) |> distinct()
    ),
    by = join_by(pid)
  ) |> 
  filter(mITT) |> 
