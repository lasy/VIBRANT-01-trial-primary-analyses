...
available_data <- 
  ...
  left_join(
    mae@sampleMap |> 
      ...
      dplyr::select(.sample, assay),
    by = join_by(.sample)
  ) |> 
  left_join(
    mae@colData |> as_tibble() |> dplyr::select(.sample, pid, site, arm, starts_with("visit_"))
  ) |> 
  mutate(available_data = ifelse(attended == FALSE, FALSE, TRUE))
...
available_data |> 
  filter(attended == TRUE) |>  # ou available_data == TRUE
...
mae[["mg_ksanity"]] |> 
  left_join(
    colData(mae) |> 
      as.tibble() |> 
      dplyr::select(.sample, site, arm, visit_code, pid), 
    by = ".sample"
  ) |> 
  filter(arm == "Pl" & !is.na(LBP)) |> 
  ggplot(aes(x = pid, y = unique_kmers, color = strain)) + 
  geom_point() + 
  facet_grid(visit_code ~ strain, scales = "free", space = "free") + 
