...
col_by_week5 <- 
  ...
  mutate(LBP_colonization_by_week5 = primary) |> 
  mutate(
    arm = 
      case_when(
        (group4 == "Yes") ~ "LC-106-o",
  ...
  mutate(site = ifelse(location == "US", "MGH", "CAP")) 
...
col_by_week5 |> 
  ...
  ungroup() |> 
  ggplot(aes(x = participant_number, y = site, col = site, fill = site, shape = LBP_colonization_by_week5)) +
  geom_point(size = 3) +
...
fit <- glm(LBP_colonization_by_week5 ~ arm + site + arm:site, data = col_by_week5, family = binomial)
fit |> summary()
...
tmp <- 
  ...
  bind_rows(
    col_by_week5 |> 
      ...
      distinct() |> 
      mutate(LBP_colonization_by_week5 = TRUE),
    col_by_week5 |> 
      dplyr::select(arm, site) |> 
      distinct() |> 
      mutate(LBP_colonization_by_week5 = FALSE)
  ) 

fit <- glm(LBP_colonization_by_week5 ~ arm + site + arm:site, data = tmp, family = binomial)
fit |> summary()
...
for (selected_site in unique(col_by_week5$site)) {
  for (selected_arm in levels(col_by_week5$arm)[-1]) {
    tmp <-
      col_by_week5 |>
      filter(site == selected_site, arm %in% c("Pl", selected_arm)) |>
      mutate(arm = arm |> fct_drop()) |>
      dplyr::select(LBP_colonization_by_week5, arm) 
    
    if (length(unique(tmp$arm)) > 1 ){
      ft <- tmp |> table() |> fisher.test()
      fisher_test_res <-
        fisher_test_res |>
        bind_rows(
          tibble(
...
fisher_test_res |>
  group_by(site) |>
  gt(caption = "Fisher exact test results", row_group_as_column = TRUE)
