...
bind_rows(
  ...
  df_summ_pid |> filter(arm != "Pl", arm != "LC106-3") |> mutate(arm = "all active arms excl. LC106-3"),
  df_summ_pid |> filter(arm != "Pl", arm != "LC106-3") |> mutate(site = "both", arm = "all active arms excl. LC106-3")
) |> 
  filter(mITT) |> 
  group_by(arm, site) |> 
  summarize(
