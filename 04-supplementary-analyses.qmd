...
We also note that two participants returned eight applicators. In both cases, comments explain that they had received a spare applicator:
...
rBV_stats |> filter(active_arm) |> dplyr::count(active_arm, BV_cure_post_MTZ, LBP_colonization_by_week5, BV_by_week12)
...
detected_qPCR |> 
  filter(visit_code %in% c("1300","2120")) |>
  filter(arm != "Pl") |> 
  group_by(visit_code) |>
  summarise(
    n    = sum(detected),
