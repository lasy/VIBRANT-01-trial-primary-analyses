...
g_trajectories <-
  ...
  facet_nested(
    ...
    strip = 
      strip_nested(
        background_x = elem_list_rect(fill = c(rep("lightgrey",5), 
                                               rep(c("#FF0000", "#00868B"), 5))
                                      ), 
        text_x = elem_list_text(colour = c(rep("black", 5), rep("transparent", 10)))              
        )
  )+
  scale_fill_manual(
...
g_trajectories + theme_bw() + theme(axis.ticks.x = element_blank(), strip.text.y = element_text(angle = 0))
