...
ordered_participants <- 
  trajectories_data |> 
  select(arm, site, pid, mITT, PP) |> 
  distinct() |> 
  arrange(arm, site, pid)

export_dir <- str_c(dropbox_dir(), "90_VIBRANT_consolidated_data/91 misc data exports/")
...
save(
  ordered_participants,
  file = str_c(export_dir, "trajectories_participants.RData")
  )

write_csv(
  ordered_participants,
  file = str_c(export_dir, "trajectories_participants.csv")
  )
...
Same analyze as above but excluding the week 2 visit for the LC-106-7 and LC-115 arms, since this visit is immediately after product use.
