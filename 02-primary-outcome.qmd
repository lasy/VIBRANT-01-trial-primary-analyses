...
col_by_week5_qPCR <-
  ...
  left_join(
    mae[["col_LBP_qPCR"]] |> 
      ...
      dplyr::filter(visit_code == "1500", .feature == "detected_by_qpcr")
  ) |> 
  mutate(LBP_colonization_by_week5 = outcome |> replace_na(FALSE)) 
...
col_by_week5 <- 
  ...
  mutate(
    arm = 
      case_when(
        (arm == "LC-106-o") ~ arm,
        (site == "MGH") ~ sample(c("Pl", "LC-106-7", "LC-115"), n(), replace = TRUE),
        (site == "CAP") ~ sample(c("Pl", "LC-106-7", "LC-106-3", "LC-115"), n(), replace = TRUE),
        TRUE ~ "Blinded"
      ) |> 
...
test_results <- 
  fisher_test_results |> 
  mutate(
    adj_p_value = adj_p_value |> round(3) |> as.character(),
    OR = OR |> round(2) |> as.character(),
    CI = str_c(lower_CI |> round(2), " - ", upper_CI |> round(2))
    ) |> 
